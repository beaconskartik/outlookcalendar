		...
		extends AsyncQueryHandler
{
	...
	public final void startQuery(Object cookie, long startTimeMillis, long endTimeMillis) {
		...
		startQuery(0, cookie, CalendarContract.Events.CONTENT_URI,
				OutlookEventCursor.PROJECTION, SELECTION, args, SORT);
	}
	
	@Override
	protected final void onQueryComplete(int token, Object cookie, Cursor cursor) {
		handleQueryComplete(token, cookie, new OutlookEventCursor(cursor));
	}
	
	protected abstract void handleQueryComplete(int token, Object cookie, OutlookEventCursor cursor);
}
